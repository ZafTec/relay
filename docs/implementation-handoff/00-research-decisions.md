...
Per [ADR 0001](../adr/0001-mcp-auth-via-better-auth-mcp-plugin.md), the owner
selected Better Auth's `mcp` convenience package (`@better-auth/mcp`, current
`1.7.1`) plus the mandatory `jwt()` plugin over a hand-built direct OAuth 2.1
provider. This is not the historical deprecated MCP plugin. `mcp()` supplies
`/oauth2/authorize`, `/oauth2/token`, `/oauth2/userinfo`, optional
