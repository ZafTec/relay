...
- BullMQ (via ioredis, per ADR 0002) transports outbox-relayed execution tickets
  ...
  -- proven live against compiled-binary BullMQ and real Redis, not yet wired
  into the API's admission path or the worker process.
...
| Tool runs                      | Implemented (schema only) | Wave 3.0 canonical schema live in PostgreSQL 18: `relay.tool_runs`, `execution_jobs`, `job_attempts`, `capacity_pools`/`capacity_policies`, the three queue-counter tables, `execution_capacity_leases`, `outbox_events`, `idempotency_records`. Verified with a real insert chain (run -> job -> attempt -> outbox event) and constraint checks (status CHECK, attempt-number uniqueness).                                                                                                                                                                                                                                                                                      | No application code performs the full admission transaction yet (idempotency resolve, counter locking in order, reservation, outbox insert as one commit) -- `packages/queue`/`packages/capacity` exist but aren't called from an HTTP handler. `tool_version_id`/`tool_id` have no FK pending the Wave 3B catalog. |
