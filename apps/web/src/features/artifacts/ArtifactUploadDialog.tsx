...
export interface ArtifactUploadDialogProps {
  readonly adapter: Pick<
    ArtifactsAdapter,
    "createUpload" | "putUpload" | "completeUpload"
  >;
...
function statusMessage(stage: UploadStage): string {
  switch (stage) {
    case "hashing":
      return "Checking your file before uploading it.";
    case "reserving":
      return "Making sure there is enough storage for your file.";
    case "uploading":
      return "Saving your file securely. Keep this window open until it finishes.";
    case "verifying":
      return "Checking that your file arrived safely.";
    case "pending":
      return "Your file has arrived. Relay is finishing a few checks.";
    case "completed":
      return "Your file is saved and ready to use.";
    case "error":
      return "The upload did not complete.";
    case "ready":
      return "Choose a file and review its details.";
  }
}
...
export function ArtifactUploadDialog({
  ...
  const busy = importBusy || stage === "hashing" ||
    stage === "reserving" ||
    stage === "uploading" ||
    stage === "verifying";
  ...
  function closeDialog(discardFrozen = false) {
    if (busy) return;
    if (operationRef.current !== null && completed === null && !discardFrozen) {
      return;
    }
    if (completed !== null) {
  ...
  function validate(): UploadErrors {
    ...
    if (artifact === undefined) {
      ...
      if (trimmedName.length === 0) next.name = "Enter an artifact name.";
        ...
        next.name = "Artifact name must be 255 characters or fewer.";
      }
      if (!MEDIA_KIND_PATTERN.test(mediaKind.trim())) {
        next.mediaKind =
          "Use a lowercase media kind with letters, numbers, dots, underscores, or hyphens.";
      }
    }
    const trimmedMimeType = mimeType.trim();
    if (trimmedMimeType.length === 0) {
      next.mimeType = "Enter the file MIME type.";
    } else if (
      trimmedMimeType.length > 255 || !MIME_TYPE_PATTERN.test(trimmedMimeType)
    ) {
      next.mimeType =
        "Enter a valid MIME type such as image/png or text/plain.";
    }
    return next;
  }
  ...
  async function completeUpload(
    operation: FrozenUploadOperation,
    generation: number,
  ) {
    ...
    try {
      const result = await adapter.completeUpload(
        operation.upload.id,
        operation.completeKey,
      );
      if (!isCurrent(generation)) return;
      if (result.kind === "auth-expired") {
        fail(
          "Your session expired before Relay could confirm completion.",
          null,
          generation,
        );
        onAuthExpired();
        ...
        fail(
          `Relay could not verify the uploaded bytes (${result.reason}).`,
          null,
          generation,
        );
      } else if (result.kind === "idempotency-conflict") {
        fail(
          "The completion idempotency key conflicts with a different request. The frozen completion request cannot be retried.",
          null,
          generation,
        );
      } else if (result.kind === "not_found") {
        fail(
          "Relay could not find the upload reservation during completion.",
          null,
          generation,
        );
      } else {
  ...
  async function uploadBytes(
    ...
    if (authorization === undefined || authorization === null) {
      fail(
        "The frozen signed upload authorization is unavailable.",
        null,
        generation,
      );
      return;
    ...
    try {
      ...
      if (result.kind === "uploaded") {
        ...
        fail(
          "The signed upload authorization expired before Relay confirmed the file transfer.",
          null,
          generation,
        );
      } else if (result.kind === "rejected") {
        fail(
          `The signed upload request was rejected with HTTP status ${result.status}.`,
          null,
          generation,
        );
      } else {
  ...
  async function reserveUpload(
    ...
    try {
      ...
      if (result.kind === "auth-expired") {
        fail(
          "Your session expired before Relay could reserve the upload.",
          null,
          generation,
        );
        onAuthExpired();
        ...
        if (result.upload.status === "completed") {
          ...
          fail(
            `The retained upload reservation is ${result.upload.status}.`,
            null,
            generation,
          );
        } else {
        ...
        fail(
          "The upload reservation idempotency key conflicts with a different request. The frozen reservation cannot be retried.",
          null,
          generation,
        );
      } else if (result.kind === "quota-exceeded") {
        fail(
          "The upload quota was exceeded. No file bytes were sent.",
          null,
          generation,
        );
      } else if (result.kind === "not_found") {
        fail(
          "Relay could not find the requested upload target. No file bytes were sent.",
          null,
          generation,
        );
      } else {
  ...
  const title = artifact === undefined
    ? "Upload artifact"
    : `Upload a new version of ${artifact.name}`;
  ...
  return (
    ...
    >
      ...
      >
        <div className="share-dialog__header">
          <div>
            <h2
              ...
              tabIndex={stage === "pending" || stage === "completed" ||
                  stage === "error"
                ? -1
                : undefined}
            >
        ...
        {artifact === undefined && completed === null && stage === "ready"
          ? (
            <div
              className="artifact-upload-source"
              role="group"
              aria-label="File source"
            >
              <Button
                variant={source === "file" ? "accent" : "outline"}
              ...
              <Button
                variant={source === "url" ? "accent" : "outline"}
        ...
        {completed !== null
          ? (
            <section
              className="artifact-upload-complete"
              aria-labelledby={`${id}-complete-title`}
            >
              <div
                className="artifact-upload-status artifact-upload-status--success"
                id={statusId}
                role="status"
                aria-live="polite"
              >
                <span
                  className="artifact-upload-status__marker"
                  aria-hidden="true"
                />
                <div>
                  <h3 id={`${id}-complete-title`}>
              ...
              <dl className="artifact-upload-summary">
                <div>
                  <dt>Artifact</dt>
                  <dd>
                    <code>{completed.artifactId}</code>
                  </dd>
                </div>
                <div>
                  <dt>Version</dt>
                  <dd>
                    <code>{completed.artifactVersionId}</code>
                  </dd>
                </div>
              </dl>
              <div className="artifact-upload-complete__actions">
                ...
                <Link
                  ...
                  to={`/dashboard/artifacts/${
                    encodeURIComponent(completed.artifactId)
                  }`}
                  onClick={() => closeDialog()}
                >
          ...
          ? (
            <ImportUrlForm
              ...
              onCompleted={(artifactId, artifactVersionId) => {
                setCompleted({
                  artifactId,
                  artifactVersionId,
                  replayed: false,
                ...
                setStage("completed");
                setMessage("File imported and saved in your workspace.");
              }}
            />
          )
          : (
            ...
            >
              ...
              {stage !== "ready"
                ? (
                  <div
                    className={`artifact-upload-status${
                      stage === "error" ? " artifact-upload-status--error" : ""
                    }`}
                    id={statusId}
              ...
              <fieldset disabled={stage !== "ready"}>
                ...
                <div className="artifact-upload-field artifact-upload-field--file">
                  ...
                  {errors.file
                    ? (
                      <p
                        id={`${id}-file-error`}
                        className="artifact-upload-field__error"
                        role="alert"
                      >
                        {errors.file}
                      </p>
                    ...
                    ? (
                      <p
                        id={`${id}-file-summary`}
                        className="artifact-upload-field__hint"
                      >
                    ...
                    <div className="artifact-upload-form__pair">
                      ...
                      <div className="artifact-upload-field">
                        ...
                        <input
                          ...
                          aria-describedby={errors.mediaKind
                            ? `${id}-media-kind-error`
                            : undefined}
                        />
                        {errors.mediaKind
                ...
                <div className="artifact-upload-field">
                  ...
                  <input
                    ...
                    aria-describedby={errors.mimeType
                      ? `${id}-mime-type-error`
                      : `${id}-mime-type-hint`}
                  />
                  {errors.mimeType
                    ...
                    : (
                      <p
                        id={`${id}-mime-type-hint`}
                        className="artifact-upload-field__hint"
                      >
                        Review this value when the browser does not identify the
                        file type.
                      </p>
                    )}
                </div>
