...
export function WorkspaceSwitcher() {
  ...
  async function switchTo(workspace: SwitchableWorkspace) {
    ...
    try {
      ...
      if (alive.current) {
        setError("The active workspace could not be changed. Try again.");
      }
    } finally {
  ...
  if (auth.workspace.status === "degraded") {
    return (
      <div className="workspace-switcher" role="status">
        <span
          className="workspace-switcher__avatar workspace-switcher__avatar--warn"
          aria-hidden="true"
        >
          ▲
        </span>
  ...
  if (!active) {
    return (
      <Link
        className="workspace-switcher workspace-switcher--empty"
      ...
      {open
        ? (
          <div className="workspace-switcher__panel" ref={panelRef}>
              ...
              {(items ?? [active]).map((workspace) => (
                <li key={workspace.id} role="presentation">
                  <button
                    ...
                    tabIndex={workspace.id === (focusedId ?? active.id)
                      ? 0
                      : -1}
                  ...
                  >
                    <Avatar
                      className="workspace-switcher__avatar"
                      src={workspace.logo}
                      fallback={initial(workspace.name)}
                    />
                    <span className="workspace-switcher__option-name">
                      {workspace.name}
                    </span>
            ...
            >
              <span
                className="workspace-switcher__avatar workspace-switcher__avatar--ghost"
                aria-hidden="true"
              >
                ＋
              </span>
