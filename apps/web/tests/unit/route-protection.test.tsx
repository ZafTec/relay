  ...
  vi.stubGlobal(
    ...
    vi.fn<typeof fetch>(async (input) => {
      ...
      if (path.startsWith("/api/v1/tools")) {
        return new Response(
          JSON.stringify({ kind: "ok", items: [], nextCursor: null }),
          {
            status: 200,
            headers: { "content-type": "application/json" },
          },
  ...
  vi.stubGlobal(
    ...
    vi.fn<typeof fetch>(async (input) => {
      ...
      if (path === "/health/ready") {
        return new Response(
          JSON.stringify({ service: "api", status: "ok", checks: [] }),
          {
            status: 200,
            headers: { "content-type": "application/json" },
          },
      ...
      if (path === "/version") {
        return new Response(
          JSON.stringify({ version: "test", revision: "test-revision" }),
          {
            status: 200,
            headers: { "content-type": "application/json" },
          },
...
describe("protected routing", () => {
  it("redirects an anonymous dashboard request and preserves its local return path", async () => {
    ...
    render(
      <App
        router={router}
        adapter={createTestAuthAdapter({ identity: null })}
      />,
    );
    ...
    expect(router.state.location.search).toContain(
      "returnTo=%2Fdashboard%3Fview%3Dcurrent",
    );
  });
  ...
  it("expires a protected route at the session's known deadline", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Sign in to Relay" }))
      .toBeInTheDocument();
    expect(router.state.location.search).toContain("reason=session-expired");
  });
  ...
  it("labels unavailable providers and lets the user retry a degraded session check", async () => {
    ...
    render(
      <App router={createRelayMemoryRouter(["/sign-in"])} adapter={adapter} />,
    );

    expect(await screen.findByRole("button", { name: "Google unavailable" }))
      .toBeDisabled();
    expect(screen.getByRole("button", { name: "GitHub unavailable" }))
      .toBeDisabled();
    ...
    expect(await screen.findByRole("button", { name: "Continue with Google" }))
      .toBeEnabled();
    expect(screen.getByRole("button", { name: "Continue with GitHub" }))
      .toBeEnabled();
  ...
  it.each([
    ...
    "renders public route %s without authentication",
    async (path, heading) => {
      mockPublicEndpoints();
  ...
  it.each([
    ...
    "renders protected application route %s",
    async (path, heading, navigationLabel) => {
      ...
      expect(screen.getAllByRole("link", { name: navigationLabel })).not
        .toHaveLength(0);
      expect(router.state.location.pathname).toBe(path);
    },
  );
  ...
  it("renders the protected dashboard with real adapter data and no invented counts", async () => {
    vi.stubGlobal(
      ...
      vi.fn<typeof fetch>(async (input) => {
        ...
        return new Response(
          JSON.stringify(emptyOverviewResponse),
    ...
    expect(await screen.findByText("Your workspace is ready"))
      .toBeInTheDocument();
    expect(screen.getAllByText("Relay test workspace").length).toBeGreaterThan(
      0,
    );
