...
const browserPublicRelease = {
  ...
  summary:
    "A test-only published release used for responsive browser coverage.",
  items: [
...
async function mockAuthenticatedWorkspace(
  ...
  await page.route("**/api/v1/overview", (route) => route.fulfill({ json: emptyOverviewResponse }));
  await page.route("**/api/v1/workspaces/connections", (route) => route.fulfill({ json: { items: [] } }));
  ...
  await page.route("**/api/v1/workspaces", (route) =>
    route.fulfill({
      json: {
        items: [{ ...workspace, role: "owner", personal: true }],
    ...
    const body = pathname === "/api/v1/tools"
      ...
      : {
        ...
        tool: {
          ...
          outputSchema: {
            type: "object",
            properties: { artifactIds: { type: "array" } },
          },
        },
      };
    await route.fulfill({
  ...
  await page.route("**/api/v1/artifacts**", async (route) => {
    ...
    const body = pathname === "/api/v1/artifacts"
      ...
      : {
        ...
        artifact: {
          ...browserArtifact,
          versions: [browserArtifactVersion],
          shares: [],
        },
      };
    await route.fulfill({
    ...
    if (new URL(route.request().url()).pathname === "/api/v1/usage/storage") {
      await route.fulfill({
        json: {
          kind: "ok",
          storage: {
            generatedAt: browserUsage.generatedAt,
...
async function expectNoPageOverflow(page: Page) {
  ...
  expect(
    ...
    `Horizontal overflow: ${
      JSON.stringify({ offenders: overflow.offenders, layout: overflow.layout })
    }`,
...
const productionTools = [
  {
    key: "image.generate.gpt-image-2",
    name: "GPT Image 2",
    category: "image",
    summary: "Generate images from a text prompt.",
  },
  {
    key: "image.generate.flux-2-pro",
    name: "FLUX.2 Pro",
    category: "image",
    summary: "Generate images with optional reference images.",
  },
  {
    key: "document.ocr",
    name: "Document OCR",
    category: "document",
    summary: "Extract text, tables, and structured data from a document.",
  },
  {
    key: "image.edit.gpt-image-2",
    name: "GPT Image 2 Edit",
    category: "image",
    summary: "Edit with reference images and an optional mask.",
  },
  {
    key: "image.edit.flux-2-pro",
    name: "FLUX.2 Pro Edit",
    category: "image",
    summary: "Edit with reference images.",
  },
  {
    key: "image.generate.mai-image-2.5",
    name: "MAI Image 2.5",
    category: "image",
    summary: "Generate a PNG image.",
  },
  {
    key: "image.edit.mai-image-2.5",
    name: "MAI Image 2.5 Edit",
    category: "image",
    summary: "Edit a PNG or JPEG image.",
  },
  {
    key: "image.generate.mai-image-2.5-flash",
    name: "MAI Image 2.5 Flash",
    category: "image",
    summary: "Generate a PNG image.",
  },
  {
    key: "image.edit.mai-image-2.5-flash",
...
async function captureReview(page: Page, name: string) {
  ...
  await page.screenshot({
    path: path.join(
      "artifacts",
      `pr35-${process.env.RELAY_UI_REVIEW_PHASE}`,
      `${name}.png`,
    ),
    fullPage: true,
  });
...
test("MVP tool composers remain accessible and usable on desktop and mobile", async ({ page }) => {
  ...
  for (const width of [1440, 768, 390]) {
    ...
    for (const tool of productionTools) {
      ...
      await expect(
        page.getByRole("heading", { name: "Create run", exact: true }),
      ).toBeVisible();
      await expect(
        page.getByRole("button", { name: "Create run", exact: true }),
      ).toBeEnabled();
...
test("creating a run brings an accessible confirmation into view", async ({ page }) => {
  ...
  await page.route("**/api/v1/runs", async (route) => {
    ...
    await route.fulfill({
      status: 202,
      headers: { location: `/api/v1/runs/${runId}` },
      json: {
        kind: "accepted",
        replayed: false,
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    for (const width of [1440, 390]) {
      ...
      await expect(
        page.getByRole("button", { name: "Creating run", exact: true }),
      ).toBeDisabled();
      ...
      await expect(confirmation.getByText("Queued", { exact: true }))
        .toBeVisible();
      await expect(page.getByLabel("Prompt", { exact: true })).toHaveValue(
        "A quiet mountain lake",
      );
      await page.keyboard.press("Tab");
      await expect(confirmation.getByRole("link", { name: "View run" }))
        .toBeFocused();
      await expectNoPageOverflow(page);
      await expectNoSeriousAxeViolations(page);
      await page.screenshot({
        path: path.join(
          "artifacts",
          "review",
          `run-accepted-${colorScheme}-${width}.png`,
        ),
      });
      await page.keyboard.press("Enter");
...
test("MVP upload and sign-out dialogs retain focus and fit small screens", async ({ page }) => {
  ...
  for (const width of [1440, 390]) {
    ...
    await page.getByRole("button", { name: "Upload artifact", exact: true })
      .click();
    ...
    await expect(dialog.getByLabel("Artifact name", { exact: true }))
      .toHaveValue("quarterly-report.pdf");
    await expect(dialog.getByLabel("Media kind", { exact: true })).toHaveValue(
      "document",
    );
    ...
    await page.keyboard.press("Tab");
    await expect(page.getByRole("button", { name: "Cancel", exact: true }))
      .toBeFocused();
...
test("MVP capacity policies remain readable across desktop and mobile", async ({ page }) => {
  ...
  await page.route("**/api/v1/admin/capacity-policies**", async (route) => {
    const pathname = new URL(route.request().url()).pathname;
    await route.fulfill({
      json: pathname === "/api/v1/admin/capacity-policies"
        ? { policies: [policy] }
        : { policy },
    });
  });
  for (const width of [1440, 768, 390]) {
    ...
    await expect(
      page.getByRole("heading", { name: "Capacity policies", exact: true }),
    ).toBeVisible();
    await expect(
...
test("landing explains the product honestly across required widths", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await expect(page.getByRole("heading", { level: 1 })).toHaveText(
      /Metered tools for agents/,
    );
    await expect(page.getByRole("heading", { name: "Check your tool access" }))
      .toBeVisible();
    await expectNoPageOverflow(page);
  }
...
test("FAQ answers are keyboard accessible in both themes and on mobile", async ({ page }) => {
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    ...
    for (const width of [390, 1440]) {
      ...
      if (width === 390) {
        await page.locator(".public-nav__menu > summary").click();
      }
      ...
      const question = faq.locator("summary").filter({
        hasText: "Does signing in give me tool access?",
      });
      const answer = faq.getByText(
        /Signing in creates or resumes your personal workspace/,
      );
...
test("compact settings and workspace switching preserve keyboard and mouse actions", async ({ page }) => {
  ...
  await page.route("**/api/v1/workspaces", (route) =>
    route.fulfill({
      json: {
        items: [workspace, other].map((item) => ({
          ...item,
          role: "owner",
          personal: item.id === workspace.id,
        })),
        maxOwnedWorkspaces: 20,
      },
    }));
  await page.route("**/api/auth/get-session**", (route) =>
    route.fulfill({
      json: {
        ...identity,
        session: { ...identity.session, activeOrganizationId: selected.id },
      },
    }));
  await page.route(
    "**/api/auth/organization/get-organization**",
    (route) => route.fulfill({ json: selected }),
  );
  await page.route("**/api/auth/organization/set-active", (route) => {
    ...
    selected = route.request().postDataJSON().organizationId === other.id
      ? other
      : workspace;
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    ...
    for (const width of [390, 1440]) {
      ...
      await rail.getByRole("button", { name: selected.name, exact: true })
        .click();
      ...
      await page.screenshot({
        path: path.join(
          "artifacts",
          "review",
          `switcher-${colorScheme}-${width}.png`,
        ),
      });
      await page.keyboard.press("Escape");
      await expect(
        rail.getByRole("button", { name: selected.name, exact: true }),
      ).toBeFocused();
      for (
        const label of [
      ...
      await expect(
        page.getByRole("table", { name: "Supported MCP authorization scopes" }),
      ).toBeVisible();
      await expect(page.getByRole("checkbox", { name: /Run completed/ }))
        .toBeEnabled();
      await expectNoPageOverflow(page);
      await expectNoSeriousAxeViolations(page);
      await page.screenshot({
        path: path.join(
          "artifacts",
          "review",
          `settings-expanded-${colorScheme}-${width}.png`,
        ),
        fullPage: true,
      });
    }
  ...
  await page.goto("/dashboard/settings");
  const trigger = page.locator(".workspace-switcher:visible").getByRole(
    "button",
    { name: workspace.name, exact: true },
  );
  await trigger.focus();
  await page.keyboard.press("ArrowDown");
  await expect(page.getByRole("option")).toHaveCount(2);
  await page.keyboard.press("End");
  await expect(page.getByRole("option", { name: /Second workspace/ }))
    .toBeFocused();
  await page.keyboard.press("Enter");
  await expect(page.getByRole("alert")).toHaveText(/could not be changed/);
  await page.getByRole("option", { name: /Second workspace/ }).click();
  await expect(
    page.locator(".workspace-switcher:visible").getByRole("button", {
      name: "Second workspace",
      exact: true,
    }),
  ).toBeVisible();
...
test("sign-in is OAuth-only and anonymous dashboard navigation is protected", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await expect(page.getByRole("heading", { name: "Sign in to Relay" }))
      .toBeVisible();
...
test("dashboard uses session, workspace, and API responses without fake counts", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await expect(
      page.locator(".workspace-switcher:visible").getByText(
        "Browser workspace",
      ),
    ).toBeVisible();
    await expect(page.getByText("Your workspace is ready")).toBeVisible();
    await expect(page.getByRole("heading", { name: "Overview", exact: true }))
      .toBeVisible();
...
test("product resource routes expose real contract data across required widths", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Tools" }))
      .toBeVisible();
    await expect(page.getByRole("link", { name: /Test-only image tool/ }))
      .toBeVisible();
    await expectNoPageOverflow(page);

    await page.goto(`/dashboard/tools/${browserTool.key}`);
    await expect(
      page.getByRole("heading", { level: 1, name: browserTool.name }),
    ).toBeVisible();
    await expect(
      page.getByText(/This tool is not ready to run yet/i),
    ).toBeVisible();
    await expectNoPageOverflow(page);
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Runs" }))
      .toBeVisible();
    await expect(page.getByRole("link", { name: `Open run ${runId}` }))
      .toBeVisible();
    await expectNoPageOverflow(page);

    await page.goto(`/dashboard/runs/${runId}`);
    await expect(
      page.getByRole("heading", { level: 1, name: browserRun.tool.name }),
    ).toBeVisible();
    await expect(page.getByRole("link", { name: artifactId })).toBeVisible();
    await expectNoPageOverflow(page);

    await page.goto("/dashboard/artifacts");
    await expect(page.getByRole("heading", { level: 1, name: "Artifacts" }))
      .toBeVisible();
    await expect(
      page.getByRole("link", { name: "Open artifact Test-only artifact" }),
    ).toBeVisible();
    await expectNoPageOverflow(page);

    await page.goto(`/dashboard/artifacts/${artifactId}`);
    await expect(
      page.getByRole("heading", { level: 1, name: "Test-only artifact" }),
    ).toBeVisible();
    await page.locator("summary").filter({ hasText: "Version history" })
      .click();
    await expect(page.getByRole("table", { name: /Immutable versions/ }))
      .toBeVisible();
    await expectNoPageOverflow(page);

    await page.goto("/dashboard/usage");
    await expect(page.getByRole("heading", { level: 1, name: "Usage" }))
      .toBeVisible();
    await expect(
      page.getByRole("table", { name: /Current consumed and reserved usage/ }),
    )
      .toBeVisible();
    ...
    await expect(
      page.getByRole("region", { name: "Your workspaces" }).getByText(
        "@browser-workspace",
      ),
    ).toBeVisible();
    await expect(page.getByRole("link", { name: "Manage OAuth clients" }))
      .toBeVisible();
    await expectNoPageOverflow(page);
  }

  for (
    const route of ["tools", "runs", "artifacts", "usage", "settings"] as const
  ) {
    await page.setViewportSize({ width: 1440, height: 900 });
    ...
    if (route === "settings") {
      ...
      await expect.poll(() =>
        page.locator(".product-main").evaluate((element) => element.scrollTop)
      )
        .toBeGreaterThan(0);
...
test("admin changelog routes stay platform-scoped and responsive", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    const visibleScope = page.locator(
      ".admin-scope:visible, .admin-mobile-scope:visible",
    );
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "Edit 1.2.3-test" }),
    )
      .toBeVisible();
    ...
    await expect(page.getByRole("heading", { level: 1, name: "1.2.3-test" }))
      .toBeVisible();
    await expect(page.getByText("This preview is not a public page."))
      .toBeVisible();
    await expectNoPageOverflow(page);
  }
  ...
  await page.screenshot({
    path: path.join(
      ...
      "artifacts",
      "screenshots",
      "admin-changelog-editor-1440.png",
    ),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(
      ...
      "artifacts",
      "screenshots",
      "admin-changelog-preview-390.png",
    ),
    fullPage: true,
  });
});
...
test("MCP consent and workspace screens preserve signed-flow behavior", async ({ page }) => {
  ...
  const consentPath =
    "/oauth/consent?client_id=https%3A%2F%2Fclient.example%2Fmetadata.json&scope=openid%20mcp%3Atools";
  const workspacePath = "/oauth/workspace?oauth_query=signed-by-server";
  for (const width of [320, 390, 768, 1024, 1440]) {
    await page.setViewportSize({ width, height: width <= 390 ? 844 : 900 });
    await page.goto(consentPath);
    await expect(
      page.getByRole("heading", { name: "Authorize Approved MCP client" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", {
        name: "Choose where this MCP client can act",
      }),
    ).toBeVisible();
    await expect(page.getByRole("radio", { name: /Browser workspace/ }))
      .toBeChecked();
    await expectNoPageOverflow(page);
  }
  await expectNoSeriousAxeViolations(page);
...
test("public information routes stay factual, searchable, and responsive", async ({ page }) => {
  ...
  await page.getByRole("searchbox", {
    name: "Search HTTP paths and MCP tool names",
  }).fill("runs:cancel");
  await expect(page.getByRole("link", { name: "relay.runs.cancel" }))
    .toBeVisible();
  await expect(page.getByText("/api/v1/runs", { exact: true }).first())
    .toBeVisible();
...
test("operational status is available only inside the superadmin console", async ({ page }) => {
  ...
  for (const path of ["/", "/docs", "/status"]) {
    ...
    await expect(page.locator('a[href="/status"], a[href="/admin/status"]'))
      .toHaveCount(0);
  }
  await expect(
    page.getByRole("heading", { name: "This Relay route does not exist" }),
  ).toBeVisible();
  ...
  page.on("request", (request) => {
    if (/\/(health\/ready|version)$/.test(request.url())) {
      diagnostics.push(request.url());
    }
  });
  await page.goto("/admin/status");
  await expect(page.getByRole("heading", { name: "Admin access unavailable" }))
    .toBeVisible();
  ...
  for (const width of [390, 1440]) {
    ...
    await page.goto("/admin/status");
    await expect(
      page.getByRole("heading", { name: "All reported checks operational" }),
    ).toBeVisible();
    await expect(page.getByText("browser-revision", { exact: true }))
      .toBeVisible();
    await page.getByText("What this page can verify", { exact: true }).click();
    await expect(page.getByText(/historical uptime percentages/i))
      .toBeVisible();
    await expectNoPageOverflow(page);
    await expectNoSeriousAxeViolations(page);
    await page.screenshot({
      path: path.join("artifacts", "review", `admin-status-${width}.png`),
      fullPage: true,
    });
  }
});
...
test("profile is protected and exposes only current account facts", async ({ page }) => {
  ...
  for (const width of [320, 390, 768, 1024, 1440]) {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Profile" }))
      .toBeVisible();
...
test("reduced motion, forced colors, and 200 percent zoom remain usable", async ({ page }) => {
  ...
  await page.setViewportSize({ width: 1024, height: 900 });
  await page.goto("/");
  const animationName = await page.locator(".landing-hero__diagram img")
  ...
  await expect(page.getByRole("heading", { level: 1, name: "1.2.3-test" }))
    .toBeVisible();
  await expect(page.getByText("Security", { exact: true }).first())
    .toBeVisible();
  await expectNoPageOverflow(page);
  await expectNoSeriousAxeViolations(page);

  await page.goto(`/admin/changelog/${adminReleaseId}`);
  await expect(page.getByRole("heading", { name: "Edit 1.2.3-test" }))
    .toBeVisible();
  await page.getByRole("button", { name: "Publish" }).click();
  const dialog = page.getByRole("dialog");
  await expect(dialog).toBeVisible();
...
test("superadmin allowances support explicit grants, revocation and responsive history", async ({ page }) => {
  ...
  const grants = [
    {
      ...
      operatorUserId: "operator-browser",
      reason: "Approved studio pilot",
    },
    {
      id: "grant-images",
      ...
      createdAt: "2026-09-01T00:00:00.000Z",
      operatorUserId: "operator-browser",
      reason: "September image allowance",
    },
  ];
  const audit = [{
    id: "2",
    ...
    if (request.method() === "POST") {
      ...
      if (pathname.endsWith("/grant")) {
        expect(input).toEqual({
          ...
          effectiveAt: null,
          expiresAt: null,
          reason: "Approved OCR pilot",
        });
        ocrAmount = input.amount;
        await route.fulfill({
          json: { grantId: "grant-ocr", operation: "grant", replayed: false },
        });
        return;
      }
      expect(input).toEqual({
        grantId: "grant-images",
        reason: "Image pilot ended",
      });
      grants[1]!.revokedAt = asOf;
      audit.unshift({
        id: "3",
        at: asOf,
        action: "allowance.revoke",
        grantId: "grant-images",
        operatorUserId: "operator-browser",
        reason: input.reason,
      });
      await route.fulfill({
        json: { grantId: "grant-images", operation: "revoke", replayed: false },
      });
      return;
    }
    if (pathname.endsWith("/workspaces")) {
      await route.fulfill({ json: { items: [selected], nextCursor: null } });
      return;
    }
    if (pathname.endsWith("/grants")) {
      await route.fulfill({ json: { items: grants, nextCursor: null } });
      return;
    }
    if (pathname.endsWith("/audit")) {
      await route.fulfill({ json: { items: audit, nextCursor: null } });
      return;
    }
    await route.fulfill({
      json: {
        ...
        limits: [
          {
            key: "images.generated",
            state: grants[1]!.revokedAt ? "none" : "limited",
            amount: grants[1]!.revokedAt ? null : "100",
            consumed: "12",
            reserved: "2",
            remaining: grants[1]!.revokedAt ? null : "86",
          },
          {
            key: "ocr.requests",
            state: ocrAmount === null ? "none" : "limited",
            amount: ocrAmount,
            consumed: "0",
            reserved: "0",
            remaining: ocrAmount,
  ...
  for (
    const [name, width, height] of [["desktop", 1440, 1000], [
      ...
      900,
      1100,
    ], ["mobile", 390, 844]] as const
  ) {
    await page.setViewportSize({ width, height });
    await page.goto(`/admin/allowances?workspace=${selected.id}`);
    await expect(page.getByRole("heading", { name: "Allowances", exact: true }))
      .toBeVisible();
    await expect(page.getByText("Execution enabled", { exact: true }))
      .toBeVisible();
    await page.getByRole("combobox", { name: "Allowance", exact: true })
      .selectOption("ocr.requests");
    await page.getByRole("combobox", { name: "Limit", exact: true })
      .selectOption("finite");
    await page.getByLabel("Number of OCR requests", { exact: true }).fill("25");
    await page.getByLabel("Reason for this change", { exact: true }).fill(
      "Approved OCR pilot",
    );
    await waitForFonts(page);
    await expectNoPageOverflow(page);
    expect(
      await page.locator(".allowance-table-scroll").evaluate((element) =>
    ...
    await page.screenshot({
      path: path.join(process.cwd(), "artifacts", "allowances", `${name}.png`),
      fullPage: true,
    });
  }
  await page.getByRole("button", { name: "Add grant", exact: true }).click();
  await expect(page.getByText("Grant added. Recorded usage is preserved."))
    .toBeVisible();
  await expect(page.getByRole("row", { name: "OCR requests 25 0 0 25" }))
    .toBeVisible();
  await page.getByRole("button", { name: "Revoke images", exact: true })
    .click();
  await expect(
    page.getByRole("heading", { name: "Revoke images", exact: true }),
  ).toBeFocused();
  await page.getByLabel("Reason for this change", { exact: true }).fill(
    "Image pilot ended",
  );
  await page.getByRole("button", { name: "Revoke grant", exact: true }).click();
  await expect(page.getByText("Grant revoked. Recorded usage is preserved."))
    .toBeVisible();
  await expect(page.getByRole("row", { name: "Images Not granted 12 2 —" }))
    .toBeVisible();
});
...
test("OAuth clients, superadmin invitations and notifications work across screen sizes", async ({ page }) => {
  ...
  await page.route("**/api/auth/oauth2/**", async (route) => {
    if (route.request().url().endsWith("/get-clients")) {
      return route.fulfill({ json: clients });
    }
    if (route.request().url().endsWith("/create-client")) {
      const client = {
        ...route.request().postDataJSON(),
        client_id: "browser-fixture-client",
      };
      clients = [client];
      return route.fulfill({
        status: 201,
        json: { ...client, client_secret: "browser-fixture-secret" },
      });
    }
    ...
    return route.fulfill({
      json: {
        admins: [{
          ...
          grantedAt: "2026-09-07T00:00:00Z",
        }],
        invitations,
      },
    });
  ...
  for (const width of [1440, 390]) {
    ...
    await expect(
      page.getByRole("heading", { name: "OAuth clients", exact: true }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Create client", exact: true })
      .click();
    await page.getByLabel("Client name").fill("Browser agent");
    await page.getByLabel("Redirect URLs").fill(
    ...
    await expect(page.getByRole("heading", { name: "Save your client secret" }))
      .toBeFocused();
    await page.getByRole("button", { name: "I’ve saved the credentials" })
      .click();
    await expect(page.getByLabel("Client secret", { exact: true })).toHaveCount(
      0,
    );
    await page.goto("/admin/superadmins");
    await expect(
      page.getByRole("heading", { name: "Superadmins", exact: true }),
    ).toBeVisible();
    await page.getByLabel("Email address").fill(invitation.email);
    await page.getByRole("button", { name: "Create invitation link" }).click();
    await expect(page.getByRole("heading", { name: "Share this invitation" }))
      .toBeVisible();
    await expectNoPageOverflow(page);
    await expectNoSeriousAxeViolations(page);
    await page.screenshot({
      path: path.join("artifacts", "access-review", `superadmins-${width}.png`),
      fullPage: true,
    });
    await page.goto("/dashboard/settings");
    await expect(page.getByRole("heading", { name: "Email notifications" }))
      .toBeVisible();
    await expect(page.getByRole("checkbox", { name: /Run completed/ }))
      .toBeEnabled();
    await expectNoPageOverflow(page);
  ...
  await page.getByRole("button", { name: "Accept superadmin invitation" })
    .click();
  await expect(page.getByRole("heading", { name: "Invitation accepted" }))
    .toBeVisible();
  await expectNoSeriousAxeViolations(page);
});
