...
Supporting line: _Relay gives MCP clients durable object storage, observable
...
immutable versions, durable job state, workspace-scoped signing, OAuth-only
auth.
...
- Hero: _Files in. Work underway. URLs out._
...
- `// 03` Request to result — _The request ends. The work continues._
- `// 04` Versions — _Every write keeps its predecessor_
- `// 05` MCP tools — _Tools an MCP client can hold_
- `// 06` Deployment — _Bring your own S3-compatible store_
- `// 07` Usage & entitlements — _Limits that fail predictably_
  (`PLANNED · NOT SHIPPED`)
- `// 08` Security & isolation — _A workspace is the boundary, not a filter_
- `// 09` Changelog — three most recent published releases
- `// 10` Get started — _Point a client at Relay._
- Footer: ZafTech Solutions · Addis Ababa, Ethiopia · Terms · Privacy · Data
  processing
