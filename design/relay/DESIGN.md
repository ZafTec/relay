...
Relay is durable infrastructure for work that does not finish in one request. The
design system exists to make state legible: what was stored, what version it
...
Light mode is not a separate design: the public surface *is* the light treatment,
and the product surface is dark-only in v1.
...
byte counts. Distinctiveness comes from the *use* of type here, not the choice:
...
- Product: 260px rail + fluid main, 32px gutter, 84px page header,
  480px right detail drawer.
...
| Error | Ink border (1px + 4px left), `▲` glyph, typed message naming the failure and what was *not* changed |
...
- WCAG AA minimum for text (4.5:1) and graphics (3:1); values recorded per token.
