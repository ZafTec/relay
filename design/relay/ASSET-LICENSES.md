...
- **AWS, Cloudflare, MinIO, PostgreSQL, Redis** — no logos anywhere. Storage
  compatibility is stated as a protocol claim in text ("S3 API",
  "SELF-HOSTED", "PRESIGNED PUT / GET"). The landing page says so explicitly:
  "Provider names are omitted deliberately. Compatibility is a protocol claim,
  not an endorsement."
